...
class Network:
	...
	def print_nodes(self):
		...
		print("\nNodes on network: {}\n".format(self))
		for i in self.nodes:
			print("Node: {} {}\n".format(i.name, i.signing_key))
		return


class Event:

	def __init__(self, time, data=None, self_parent=None, other_parent=None, self_parent_event_hash=None, other_parent_event_hash=None, node):
		self.timestamp = time
		self.data = data
	...
	def check_supermajority(self, node_list, event):
		...
		if event.witness:
			...
			first = check_supermajority(node_list, event.sp[0])
			for i in first:
				if i not in node_list:

			second = check_supermajority(node_list, event.op[0])
...
class Node:
	...
	def begin_sync(self, targ_node):
		...
		if SIM:
			...
			targ_idx = find_targ_idx(targ_node)
	...
	def divide_rounds(self):
		...
		for i in self.hg[self.name]:
			if !i.sp_hash:
				...
				event_supermajority = i.check_supermajority(self, [], i)
	...
	def main(self):
		...
		while True:
			...
			if not SIM:
				...
				while(current_node == (rand_node_idx := random.randrange(N))):		# Pick random node != current node
					pass
...
def test_nodes(nw):
	...
	while True:
		...
		while(r_node == (new_node := random.choice(list(nw.nodes[current_node].hg)))):
			pass
